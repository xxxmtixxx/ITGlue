...
function Process-Relationships($relationships) {
    $processed = @{}
    foreach ($relationship in $relationships.PSObject.Properties) {
        if ($relationship.Name -in @("related-items", "passwords")) {
            if ($relationship.Value.data.Count -gt 0) {
                $processed[$relationship.Name] = ($relationship.Value.data | ForEach-Object {
                    "ID: $($_.id), Type: $($_.type)"
                }) -join "; "
            } else {
                $processed[$relationship.Name] = ""
            }
        }
    }
    return $processed
...
# Function to fetch configurations with relationships for an organization
function Get-OrganizationConfigurationsWithRelationships($org_id, $org_name) {
    $all_configurations = @()
    ...
    do {
        ...
        if ($config_response.data -ne $null) {
            foreach ($config in $config_response.data) {
                # Fetch detailed configuration data
                $detail_url = "https://api.itglue.com/configurations/$($config.id)?include=passwords,related-items"
                $detail_response = Invoke-RestMethod -Uri $detail_url -Method 'GET' -Headers $headers

                # Process relationships
                $relationships_data = Process-Relationships $detail_response.data.relationships

                # Create the configuration object with all attributes
                $config_obj = [ordered]@{}
                $config_obj['id'] = $detail_response.data.id  # Add 'id' to the beginning
                foreach ($attr in $detail_response.data.attributes.PSObject.Properties) {
                    $config_obj[$attr.Name] = $attr.Value
                }

                # Add processed relationships
                foreach ($rel in $relationships_data.GetEnumerator()) {
                    $config_obj[$rel.Key] = $rel.Value
                }
                ...
                # Debug output
                Write-Host "Organization: $org_name (ID: $org_id), Configuration ID: $($detail_response.data.id)" -ForegroundColor Cyan
            }
        }
...
if ($org_response.data -ne $null) {
    ...
    if ($selection -eq 0) {
        ...
        foreach ($org in $organizations) {
            ...
            $org_configurations = Get-OrganizationConfigurationsWithRelationships -org_id $org_id -org_name $org_name
            $all_configurations += $org_configurations
        }

        # Export all configurations to a single CSV
        $output_filename = "All_Organizations_Configurations_Relationships"
        if ($filterType -ne "") {
    ...
    elseif ($selection -ge 1 -and $selection -le $organizations.Count) {
        ...
        $output_filename = "${safe_org_name}_Configurations_Relationships"
        if ($filterType -ne "") {
            $output_filename += "_$filterValue"
        }
        $output_path = Join-Path $output_dir "$output_filename.csv"
